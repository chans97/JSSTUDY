...
function createPainter(parent, width, height) {
    // 타이틀
    var title = elt("h2", null, "Chan's Painter");
...
// * square : 사각형 그리기 도구
paintTools.squareFill = function (e, ctx) {
    var img = ctx.getImageData(0, 0, ctx.canvas.width, ctx.canvas.height);
    var p = relativePosition(e, ctx.canvas);
    setDragListeners(ctx, img, function (q) {
        var dx = q.x - p.x;
        ...
        var r = Math.sqrt(dx * dx + dy * dy);
        ctx.beginPath();
        ctx.rect(p.x, p.y, dx, dy);
        ctx.stroke();
        ctx.fill();
    });
};

// * squareFill :채워진 사각형 그리기 도구
paintTools.square = function (e, ctx) {
    var img = ctx.getImageData(0, 0, ctx.canvas.width, ctx.canvas.height);
    var p = relativePosition(e, ctx.canvas);
    setDragListeners(ctx, img, function (q) {
        var dx = q.x - p.x;
...
controls.color = function (ctx) {
    ...
    var label = elt("label", null, " 선 색：", input);
...
controls.color1 = function (ctx) {
    var input = colorInput = elt("input", { type: "color" });
    var label = elt("label", null, " 채우기 색：", input);
    input.addEventListener("change", function (e) {// 참고 : Firefox에서는 change 이벤트가 발생하지 않습니다.
